...
var series = [];
var provinceArea = [];  // 地区高亮

showData.forEach(function(item, idx) {
    provinceArea.push(
        {
            name: item.province,
            selected: false,  // selected:true 默认选中
            itemStyle: {
                areaColor: {
                    type: 'radial',
                    x: 0.5,
    ...
    series.push({
        ...
        //被攻击点
        {
            ...
            data: [{
                name: item.city,
                value: geoCoordMap[item.city].concat([100]),
            }],
        }
    );
});
